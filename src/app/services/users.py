...
from app.schemas.users import UserFilterParams, UserOrderParams, \
    UserWithTaskSchema
...
class UsersService:
    ...
    async def get_users(self, filters: UserFilterParams,
                        order: UserOrderParams) -> list[UserWithTaskSchema]:
        users = await self._user_repository.get_all(filters)
        users_ids = {user.id: [0, 0] for user in users}
        user_ids_with_tasks = await self._user_repository.count_completed_tasks_for_users(
            tuple(users_ids.keys()))
        self._aggregate_users_info(users_ids, user_ids_with_tasks)

        users_gen = (
            UserWithTaskSchema(
                **user.model_dump(), task_count=users_ids[user.id][0],
                task_progress=users_ids[user.id][1]
            )
            for user in users
        )
    ...
    def _aggregate_users_info(
            self, users_ids: dict[UUID, list[int]],
            user_ids_with_tasks: Sequence[tuple[UUID, int, int]]
    ) -> dict[UUID, list[int]]:
        for info in user_ids_with_tasks:
            ...
            users_ids[user_id][1] = UserProgress.calculate(completed_tasks,
                                                           all_tasks)
