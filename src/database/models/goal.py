...
class Goal(BaseModel, IdMixin, TsMixinCreated, TsMixinUpdated):
    ...
    goal_name: Mapped[str] = Column(String(500), nullable=False,
                                    comment="Цель развития")
    employee_side_plus: Mapped[str] = Column(String(500), nullable=False,
                                             comment="Сильные стороны")
    employee_side_minus: Mapped[str] = Column(String(500), nullable=False,
                                              comment="Зоны развития")

    def __repr__(self) -> str:
        return (f"Goal({self.id}, {self.user_id}, {self.goal_name}, "
