...
class SqlAlchemyGoalRepository(AbstractGoalRepository):
    ...
    async def create_goal_for_user(self, user_id: UUID, goal_data: dict) -> GoalSchema:
        try:
            ...
            goal_count: Optional[int] = await self._session.scalar(
    ...
    async def update_goal(self, user_id: UUID, updated_data: dict) -> GoalSchema:
        try:
            ...
            result = await self._session.execute(select(Goal).filter(Goal.user_id == user_id))
            ...
            updated_goal = await self._session.execute(select(Goal).filter(Goal.user_id == user_id))
